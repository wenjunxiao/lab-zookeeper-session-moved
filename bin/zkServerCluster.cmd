...
set ZK_URL=http://archive.apache.org/dist/zookeeper/%ZK_VERSION%/%ZK_VERSION%.tar.gz
set LAB_DIR=%LAB_HOME%tmp
set ZK_TAR_FILE="%LAB_DIR%\%ZK_VERSION%.tar.gz"
set ZOOKEEPER_DIR=%LAB_DIR%\%ZK_VERSION%
...
if not exist "%ZOOKEEPER_DIR%" (
    ...
    if exist "%ZK_TAR_FILE%" (
        java -jar %DOWNLOADER% %ZK_TAR_FILE% "%LAB_DIR%"
    ) else (
        java -jar %DOWNLOADER% %ZK_URL% "%LAB_DIR%"
    )
)
